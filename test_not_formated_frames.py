...
if __name__ == "__main__":
    ...
    ### model options
    parser.add_argument('-method',          type=str,     default='trained_model_example',  help='test model name')
    parser.add_argument('-epoch',           type=int,     default=100,              help='epoch')

    ### dataset options
    parser.add_argument('-dataset',         type=str,     default='DAVIS',          help='dataset')
    parser.add_argument('-phase',           type=str,     default='test',           choices=['train', 'test'])
    parser.add_argument('-data_dir',        type=str,     default='/home/zyz/Documents/SVCNet/Learning-Blind-Video-Temporal-Consistency-with-PWCNet/data',           help='path to data folder')
    ...
    for v in range(len(video_list)):
        ...
        print("Test on %s video %d/%d: %s" %(opts.dataset, v + 1, len(video_list), video))

        ## setup path
        input_dir = os.path.join(opts.data_dir, "input", opts.dataset, video)
        process_dir = os.path.join(opts.data_dir, "processed", opts.dataset, video)
        output_dir = os.path.join(opts.data_dir, "output", opts.dataset, video)
        ...
        frame_p1 = utils.read_img(os.path.join(process_dir, "00000.png"))
        ...
        for t in range(1, len(frame_list)):
            ...
            frame_p2 = utils.read_img(os.path.join(process_dir, "%05d.png" %(t)))
